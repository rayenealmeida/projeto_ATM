...
class AutocashApp:
    def __init__(self):
        self.self.diretorio_atual = os.path.dirname(os.path.abspath(__file__))
        self.db = TinyDB(self.self.diretorio_atual + '/banco_de_dados.json')

        self.self.janela = tk.Tk()
        self.self.janela.geometry("600x600")
        self.self.janela.title("Menu")
        self.self.janela.resizable(False, False)

        imagem = Image.open(self.diretorio_atual + "/images/atm_bg.png")
        imagem = imagem.resize((600, 600), Image.ANTIALIAS)
        imagem_tk = ImageTk.PhotoImage(imagem)
        label = tk.Label(self.self.janela, image=imagem_tk).place(x=0, y=0, relwidth=1, relheight=1)
